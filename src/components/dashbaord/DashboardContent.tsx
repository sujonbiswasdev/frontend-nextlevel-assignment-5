...
const DashboardContent = ({
  ...
  return (
    <div className="overflow-y-clip">
      ...
      <StatsCounts role={role as string} statsCount={stats.counts} />
      <VisibilityPublicPrivate statsCount={eventVisivility} />
      <EventStatusCounts
        eventStatus={stats.eventStatus as unknown as EventStatus}
      />
      <EarningChart
...
export const StatsCounts = ({
  ...
  const total=statsCount.participatedEvents +statsCount.invitations +statsCount.payments || 1
  return (
    <div className="max-w-[1380px] mx-auto w-full px-4 sm:px-6 lg:px-8 py-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        <StatsCard
          ...
          percentage={((statsCount.participatedEvents /total) *100).toFixed(0) }
          trend="up"
        />
        <StatsCard
          title="Invitations"
        ...
        {/* Only admin can see this card */}
        {role === "ADMIN" && (
          <StatsCard
            title="Users"
            value={statsCount.user?.toString() ?? "--"}
    ...
    <div className="max-w-[1380px] mx-auto w-full px-4 sm:px-6 lg:px-8 py-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        <StatsCard
          ...
          key="free-events"
          percentage={(
            (statsCount.free / (statsCount.free + statsCount.paid)) *
            100
          ).toFixed(0)}
          trend="up"
        />
        <StatsCard
          ...
          key="paid-events"
          percentage={(
            (statsCount.paid / (statsCount.free + statsCount.paid)) *
            100
          ).toFixed(0)}
          trend="up"
        />
    ...
    <div className="max-w-[1380px] mx-auto w-full px-4 sm:px-6 lg:px-8 py-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        <StatsCard
          ...
          percentage={(
            (statsCount.public /
              (statsCount.public + statsCount.private || 1)) *
            100
          ).toFixed(0)}
          trend="up"
        />
...
export const EventStatusCounts = ({
  ...
  const total =
    eventStatus.upcoming +
      ...
      eventStatus.draft +
      eventStatus.ongoing || 1; // Prevent division by zero
